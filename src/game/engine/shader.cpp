...
void Shader::cleanUp() { glDeleteProgram(m_id); }

int Shader::compileShader(GLuint& shader, char const * path) const {
    int success;
    char info_log[512];
...
std::string Shader::loadShaderSrc(char const * path) const {
    std::string ret;
...
Shader::operator GLuint() { return m_id; }